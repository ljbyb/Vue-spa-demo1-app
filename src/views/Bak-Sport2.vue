  ...
  export default {
      ...
      if (userName !== null) {
        if (userName === this.userName) {
          ...
          if (lastDate == null || lastDate !== this.nowDate) {
            ...
            this.$store.dispatch('addStep', this.nowDate);
          } else {
    ...
    methods: {
      drawLine(){
        ...
        let yMax = 100;
        let dataShadow = [];
        for (let value of this.$store.state.sports1.values()) {
        if (yMax < value) {
            yMax = ( Math.floor(value/100) + 1) * 100;
          }
        }

        for (let i = 0; i < this.$store.state.sports1.size; i++) {
        ...
        let option2 = {
          title: {
            text: '行走记录',
            subtext: '已记录' + this.$store.state.sports1.size + '天',
            x: '50%',
            y: '3%',
            textAlign: 'center'
          },
          xAxis: {
            data: Array.from(this.$store.state.sports1.keys()),
          },
          yAxis: {
            axisLine: {
              show: false
          ...
          series: [
            ...
            },
            {
              ...
              data: Array.from(this.$store.state.sports1.values())
            }
          ]
        };
      ...
      stepCounter: function () {
        ...
        function motionHandler(event) {
          ...
          if (onSensorChanged(accGravity.x, accGravity.y, accGravity.z)) {
            ...
            this.stepNumbers++;
            this.$store.dispatch('editStep', {date:this.nowDate, count:this.stepNumbers});
          }
        }
